...
impl SocketSinkConfig {
    pub fn new(mode: Mode, encoding: EncodingConfig<Encoding>) -> Self {
        SocketSinkConfig {
            mode,
            encoding: EncodingConfigAdapter::legacy(encoding),
        }
    }
...
mod test {
    ...
    async fn test_udp(addr: SocketAddr) {
        ...
        let config = SocketSinkConfig {
            ...
            encoding: EncodingConfigAdapter::new(None, JsonSerializerConfig::new().into()),
        };
    ...
    async fn tcp_stream_detects_disconnect() {
        ...
        let config = SocketSinkConfig {
            ...
            encoding: EncodingConfigAdapter::new(None, RawMessageSerializerConfig::new().into()),
        };
        let context = SinkContext::new_test();
        let (sink, _healthcheck) = config.build(context).await.unwrap();
